...
//check permissions
	if (permission_exists('mobile_code_view')) {
		//access granted
	}
	else {
		echo "access denied";
...
//delete the mobile_prefix
	if (permission_exists('mobile_code_delete') && ($action == "delete") && (strlen($id) > 0)) {
		$sql = "delete from v_mobile_code ";
		$sql .= "where mobile_prefix = '".$id."' ";
...
//prepare to page the results
	$sql = "select count(mobile_prefix) as num_rows from v_mobile_code ";
	$sql .= "where 1 = 1 ";
	if (isset($sql_search)) {
			$sql .= "and ".$sql_search;
	}
...
//show the content
	...
	echo "	<tr>\n";
	echo "		<td width='50%' align='left' nowrap='nowrap'><b>".$text['title-mobile_code']."  (".$num_rows.")</b></td>\n";
	echo "		<form method='get' action=''>\n";
	echo "			<td width='50%' style='vertical-align: top; text-align: right; white-space: nowrap;'>\n";

	if (permission_exists('mobile_code_all')) {
		if ($_GET['show'] == 'all') {
			echo "		<input type='hidden' name='show' value='all'>";
		}
		else {
			echo "		<input type='button' class='btn' value='".$text['button-show_all']."' onclick=\"window.location='mobile_code.php?show=all';\">\n";
		}
	}
	...
	echo "			".$text['description-mobile-code']."<br /><br />\n";
	echo "		</td>\n";
	echo "	</tr>\n";
	echo "</table>\n";
	...
	if (permission_exists('mobile_code_add')) {
		echo "		<a href='mobile_code_edit.php' alt='".$text['button-add']."'>$v_link_label_add</a>";
	}
	else {
		echo "&nbsp;\n";
	}
	...
	if (is_array($data_list)) {
		...
		foreach($data_list as $row) {
			if (permission_exists('mobile_code_edit')) {
				$tr_link = "href='mobile_code_edit.php?id=".urlencode($row['mobile_prefix'])."'";
			}
			echo "<tr ".$tr_link.">\n";
			...
			echo "		<input type='hidden' name=\"data_list[$x][mobile_prefix]\" value='".escape($row['mobile_prefix'])."' />\n";
			echo "	</td>\n";
